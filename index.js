function bomberMan(n, grid) {
  ...
  function memoDamangePositions(colum, columArr, row, rowArr) {
    memo.push([colum, row]);
    if (colum != columArr.length - 1) memo.push([colum + 1, row]);
    if (colum != 0) memo.push([colum - 1, row]);
    if (row != rowArr.length - 1) memo.push([colum, row + 1]);
    if (row != 0) memo.push([colum, row - 1]);
  }
  ...
  function detonateBombs(positionsArray) {
    const result = positionsArray.map((c, columIndex) =>
      c.map((r, rowIndex) => {
        if (memo.find(m => m[0] == columIndex && m[1] == rowIndex)) {
          return ".";
        }
        return "O";
      })
